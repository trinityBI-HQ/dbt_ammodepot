...
- **Progress gate (2026-07-22, PR #32)**: runs between the circuit breaker and the action — cancel+restart only fires when the job is NOT progressing. Reuses the `ATTEMPT_JSON` `_capture_evidence` already collects (no extra SSM call). `ACT` on: no live job · `bytesSynced=0` on a live job (classic freeze — acts immediately) · counters flat between samples · **any** missing/unparseable evidence. `SKIP` only on positive evidence of movement. Prior samples live in DynamoDB under `progress#<connection_id>` — sharing the breaker's bare key would clobber `breaker_until` (`put_item` replaces the whole item). Rollback: `PROGRESS_GATE_ENABLED=false`. Tests: `lambda/airbyte_auto_remediate/test_progress_gate.py` (9 cases, no AWS needed). **Why it exists:** on 2026-07-22 the Lambda cancelled a Magento drain that was committing — job 28578 lost 1h21m — because staleness alone can't tell frozen from slow.
