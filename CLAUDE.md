...
| Ingestion | Airbyte CDC on EC2 c6a.2xlarge → S3 Iceberg (Glue catalog). Legacy → Snowflake connections inactive 2026-04-07. Per-connector memory via connection `resourceRequirements`: **Magento 5 GiB** (raised from 3 GiB on 2026-07-22 after a heap-OOM spiral), **Fishbowl 2 GiB**. Schedules staggered 2026-07-27: Magento `0 0/10` (:00,:10,…), Fishbowl `0 5/10` (:05,:15,…) — they previously fired simultaneously. Control-plane pods bounded 2026-07-27 (see `airbyte-ec2/airbyte-values.yaml`) |
...
| Infra Monitoring | Snowsight dashboard (8 tiles) + Streamlit infra monitor app (`AD_ANALYTICS.OPS.INFRA_MONITOR`, SiS container runtime, 6 pages: SF Compute, SF Storage, AWS Infra, Combined, dbt Pipeline, Airbyte Health) |
| Airbyte Auto-Remediation | AWS Lambda `airbyte-auto-remediate` (Phase 2, EventBridge `cron(5,20,35,50)`) — autonomous cancel + restart of stuck Airbyte syncs via SSM, with DynamoDB circuit breaker, S3-LIST verification, and audit log in `AD_ANALYTICS.OPS.AIRBYTE_REMEDIATION_LOG` |
...
│   ├── snowflake_queries.py       # All ACCOUNT_USAGE SQL + Airbyte freshness queries (ttl=1m)
...
└── setup/
    ...
    └── 07_airbyte_observability.sql  # Airbyte freshness monitor: threshold table, views, alerts
```
...
- **Airbyte Observability (2026-05-01)**: Page 6 (`6_Airbyte_Health.py`) monitors S3 Iceberg ingestion freshness. Bootstrap in `setup/07_airbyte_observability.sql` (ACCOUNTADMIN, idempotent) creates: `AIRBYTE_FRESHNESS_THRESHOLDS` config table (**per-connection since 2026-07-27**: `magento_s3` 30/45, `fishbowl_s3` 60/90 — a single global pair could not fit both, see the Control-Plane OOM + Alert Thrash section; tunable via UPDATE), `V_AIRBYTE_FRESHNESS` + `V_AIRBYTE_FRESHNESS_PER_STREAM` views over all 55 LAKEHOUSE_LANDING tables, `SP_SEND_AIRBYTE_FRESHNESS_EMAIL(tier)` procedure, and two edge-triggered ALERT objects on `ETL_WH` at `cron(5,20,35,50)` — `ALERT_AIRBYTE_FRESHNESS_ALERT` armed, **`ALERT_AIRBYTE_FRESHNESS_WARN` SUSPENDED since 2026-07-27** (it produced ~27 emails in 26h and never matched an actionable fault; the WARN *status* stays in the view and on the Page 6 RAG cards, only the email is off). Re-arm with `ALTER ALERT ... RESUME`. Emails via `OPS_EMAIL_NOTIFICATIONS` (account-level EMAIL integration, recipient `victor@trinitybi.com`). Runbook: `docs/AIRBYTE_INCIDENT_RUNBOOK.md`.
...
Renamed 2026-04-15 — `AD_ANALYTICS.OPS.INFRA_MONITOR` (6 pages, container runtime — Airbyte Health page added 2026-05-01):
...
### Airbyte Observability (Snowflake-native — destination-freshness monitor)

Built + Deployed 2026-05-02 — pure Snowflake-native alert path (no AWS plumbing):
- **Detection**: `V_AIRBYTE_FRESHNESS` view computes per-connection staleness from the BUSIEST stream's `MAX(_airbyte_extracted_at)` across all 55 LAKEHOUSE_LANDING tables (idle CDC streams legitimately have stale extracts and must NOT be the connection-level signal — using MIN-of-MAX would false-alarm on idle config tables)
- **Alerting**: 2 edge-triggered Snowflake `ALERT` objects. `ALERT_AIRBYTE_FRESHNESS_ALERT` armed (magento 45 min / fishbowl 90 min); **`ALERT_AIRBYTE_FRESHNESS_WARN` suspended 2026-07-27** — it only ever fired on Fishbowl's legitimate overnight quiet, never on a real fault. Running on `ETL_WH` at `cron(5,20,35,50 * * * * UTC)` — same cadence as dbt build to piggyback on warm warehouse (~$0/mo incremental cost vs ~$216/mo for a dedicated XSMALL with cold-start minimums)
- **Email path**: account-level `OPS_EMAIL_NOTIFICATIONS` integration → `victor@trinitybi.com`. Recipient must be a verified Snowflake user (clicked the verification link, not just SET via `ALTER USER`). Generic name so future ops alerts can reuse.
- **Config**: `AIRBYTE_FRESHNESS_THRESHOLDS` table — operator can tune via `UPDATE ... SET warn_minutes=...` without code/Streamlit redeploy
- **Dashboard**: Page 6 (`6_Airbyte_Health.py`) in Infra Monitor — RAG cards per connection, per-stream detail
- **Runbook**: `docs/AIRBYTE_INCIDENT_RUNBOOK.md` — cancel + restart via SSM
- **TZ gotcha**: All timestamp casts use `to_timestamp_ltz(_airbyte_extracted_at, 3)` (NOT `to_timestamp` which yields NTZ — comparing NTZ to LTZ via DATEDIFF assumes session-tz reading and produces wrong / negative values)
- **Cost**: <$2/mo additional (synthetic test confirmed end-to-end email delivery 2026-05-02)
- **First real incident validation (2026-05-03)**: Magento connection job 7605 hung at 10:55 UTC making zero progress (`bytesSynced=0, lastUpdatedAt=null`). Monitor caught the freeze ~35 min after onset — `ALERT_AIRBYTE_FRESHNESS_WARN` TRIGGERED at 11:20 UTC, `_ALERT` TRIGGERED at 11:50 UTC; both emails delivered. Remediation executed via `aws ssm send-command` (no interactive shell needed): cancelled job 7605 (final stuck duration: 2h 9m), restarted as job 7619. End-to-end first-real-incident-to-recovery cycle ≈ 2h 15m (the freeze had already started before any human noticed, so detection latency = thresholds = 30/60 min, restart latency = a few min once the engineer was alerted)
- **2-week verification routine**: scheduled remote agent (`trig_01S1oP1NvjUdW8PfYbwsZfyJ`) fires 2026-05-15 to post a checklist comment on ClickUp 86ah7wvyy with verification queries
...
### Airbyte Observability (2026-05-01)

Built + Shipped 2026-05-01 — destination-freshness monitor for both Iceberg connections:
- **Page 6**: `6_Airbyte_Health.py` in Streamlit Infra Monitor — RAG cards per connection (`fishbowl_s3`, `magento_s3`), per-stream staleness table, threshold display
- **Bootstrap**: `streamlit_cost_monitor/setup/07_airbyte_observability.sql` (ACCOUNTADMIN, idempotent) creates `AIRBYTE_FRESHNESS_THRESHOLDS` config table (per-connection since 2026-07-27: `magento_s3` 30/45, `fishbowl_s3` 60/90; tunable via `UPDATE`), `V_AIRBYTE_FRESHNESS` + `V_AIRBYTE_FRESHNESS_PER_STREAM` views over all 55 LAKEHOUSE_LANDING tables, `SP_SEND_AIRBYTE_FRESHNESS_EMAIL(tier)` procedure, two edge-triggered alerts on `ETL_WH` at `cron(5,20,35,50)` to piggyback on warm warehouse (`_WARN` created SUSPENDED on purpose since 2026-07-27; `_ALERT` armed)
- **Email channel**: `OPS_EMAIL_NOTIFICATIONS` (account-level integration, recipient `victor@trinitybi.com`) — reusable for future ops alerts
- **Runbook**: `docs/AIRBYTE_INCIDENT_RUNBOOK.md` — 5-minute cancel + restart playbook (SSM-only EC2)
- **Cost**: ~$0/mo incremental (alerts run on warm ETL_WH, no extra credits)
- **Follow-up**: `/schedule` routine `trig_01S1oP1NvjUdW8PfYbwsZfyJ` posts a queries-checklist to the parent ClickUp task on 2026-05-15 (post-launch verification)

### Airbyte Auto-Remediation (Phase 2, 2026-05-03)

Built 2026-05-03 — autonomous cancel + restart of stuck Airbyte → S3 Iceberg syncs (follow-up to Phase 1 Observability):
- **Lambda**: `airbyte-auto-remediate` (container image, 512 MB, 600 s timeout, no VPC). EventBridge `cron(5,20,35,50 * * * ? *)` UTC — same cadence as dbt + Phase 1 alerts so Snowflake queries land on a warm `ETL_WH` (zero cold-start surcharge).
- **Code**: `lambda/airbyte_auto_remediate/` (first AWS Lambda in repo — establishes the deploy pattern: `Dockerfile` → ECR `ammodepot/airbyte-auto-remediate` → `aws lambda update-function-code`). Authenticates to Snowflake as `SVC_DBT` with `QUERY_TAG = 'lambda:airbyte_auto_remediate'`.
- **Networking**: SSM-only (no VPC-direct) — VPC-attached Lambda would need NAT gateway ($33/mo) and blow the $2/mo cost ceiling. Drives Airbyte EC2 (`i-075043415ebad732f`) via `ssm:SendCommand`.
...
- **Toggle**: SSM Parameter `/airbyte-auto-remediate/observe-only` — operator flips between `true` (log only) and `false` (live action) without redeploy. **Default `true` on initial deploy** for ≥1 week observe-only soak.
- **Verification**: Snowflake re-query (`V_AIRBYTE_FRESHNESS`) primary; S3 LIST on canary table (`soitem` for fishbowl, `sales_order_item` for magento) fallback. Real S3 layout is `s3://ammodepot-lakehouse/iceberg/<glue_db>.db/<table>/data/` (DESIGN doc had `<glue_db>/<table>/` — corrected during build).
- **Progress gate (2026-07-22, PR #32)**: runs between the circuit breaker and the action — cancel+restart only fires when the job is NOT progressing. Reuses the `ATTEMPT_JSON` `_capture_evidence` already collects (no extra SSM call). `ACT` on: no live job · `bytesSynced=0` on a live job **older than `MIN_ZERO_PROGRESS_JOB_AGE_SEC` (default 1500s)** · counters flat between samples · **any** missing/unparseable evidence. `SKIP` only on positive evidence of movement. Prior samples live in DynamoDB under `progress#<connection_id>` — sharing the breaker's bare key would clobber `breaker_until` (`put_item` replaces the whole item). Rollback: `PROGRESS_GATE_ENABLED=false`. Tests: `lambda/airbyte_auto_remediate/test_progress_gate.py` (16 cases, no AWS needed). **Why it exists:** on 2026-07-22 the Lambda cancelled a Magento drain that was committing — job 28578 lost 1h21m — because staleness alone can't tell frozen from slow.
- **Minimum job-age floor (2026-07-27)**: the `bytesSynced=0` branch above originally acted *immediately*, which cancelled Magento job 30043 at **83 seconds** old. Airbyte publishes byte/row counters on commit, so a healthy sync reads 0 for its entire run — and runs reach ~20 min at p99 (measured over the last ~95 succeeded jobs per connection: magento p50 168s / p90 557s / p99 1157s; fishbowl p50 116s / p90 382s / p99 1199s). Below the floor the gate now returns `SKIP_JOB_TOO_YOUNG` and records a sample, letting the flat-counters comparison decide later. Costs real-freeze detection nothing — the Lambda only inspects a connection that has *already* breached its 30–90 min staleness threshold, by which point a frozen job is far older than the floor. Missing/unparseable `startTime` still falls through to `ACT`, preserving the "absent evidence never spares a cancel" bias.
- **Restart uses the INTERNAL API, not the public one (2026-07-27)**: `POST /api/public/v1/jobs` returns **HTTP 500** for both connections and always has, because the public-API mapper NPEs on our per-connector memory caps — `NullPointerException: getDefault(...) must not be null` at `ScopedResourceRequirementsKt.toPublic` → `SourceReadMapper.from` → `SourceServiceImpl.getSource` → `ConnectionServiceImpl.getConnection`. **Every public endpoint that resolves a source or connection 500s** (`GET /api/public/v1/sources/{id}` and `/connections/{id}` both verified 500 on a healthy server). Cancel survived only because `DELETE /api/public/v1/jobs/{id}` takes a job id and never resolves the connection — so auto-remediation was a **pure kill switch**: it cancelled live syncs and could never restart them. That, not the OOM, is what produced all 11 ESCALATEs in 72h. `cancel_and_restart.json.tmpl` now posts to `/api/v1/connections/sync` (verified HTTP 200, job 30114) and parses `.job.id`. **Do not "simplify" this back to the public API** while connection-level `resourceRequirements` are set. The jobs *list* (`GET /api/public/v1/jobs?connectionId=`) is unaffected and still public.
- **Audit log**: `AD_ANALYTICS.OPS.AIRBYTE_REMEDIATION_LOG` (bootstrap: `streamlit_cost_monitor/setup/08_airbyte_remediation_log.sql`) — one row per AUTO_FIX / ESCALATE / BREAKER_OPEN / OBSERVE_ONLY_WOULD_ACT / SKIPPED_PROGRESSING event. WARN tier and OK invocations stay in CloudWatch only. Note: the `chk_outcome` CHECK constraint in that SQL is **documentation only** — Snowflake neither enforces nor registers CHECK constraints on standard tables (`information_schema` shows only the PK, itself unenforced).
- **Notifications**: SNS topic `airbyte-auto-remediate-events` → email subscription, separate from Phase 1's Snowflake EMAIL integration. Subjects: `[Airbyte AUTO-FIX]` / `[Airbyte ESCALATE]` / `[Airbyte OBSERVE]`. Plus ClickUp comment per outcome event on task `86ah8bpmj`.
- **Phase 1 independence**: disabling Lambda does NOT affect Phase 1 emails (validated by AT-007). Two layers, two delivery paths.
- **CI/CD**: `.github/workflows/deploy-lambda-airbyte-auto-remediate.yml` — path-filtered to `lambda/airbyte_auto_remediate/**`, builds + pushes container, calls `aws lambda update-function-code`. First-time provisioning still requires `lambda/airbyte_auto_remediate/deploy.sh` (idempotent — provisions ECR, SNS, DynamoDB, IAM role, SSM Parameter, Lambda, EventBridge rule, CloudWatch alarms).
- **Latency** (after Decision 2 trade-off): detection-to-action ≤16 min worst case, mean ~7.5 min (relaxed from DEFINE's ≤6 min — 5-min cron would burn $288/mo on Snowflake cold-starts).
- **Cost**: ≤$2/mo. CloudWatch billing alarm fires at $5/mo as hard cap.
...
1. **Control-plane global OOM.** All 9 platform pods shipped `resources={}` (BestEffort) while 5 run `-XX:MaxRAMPercentage=75.0` — the *same* unbounded-heap bug as Investigation B, on the other half of the cluster. Each JVM sized its heap off the host's 16 GB and inflated: measured `server` 2.44 GiB, `cron` 2.12 GiB, `workload-api-server` 1.57 GiB, **total BestEffort slice 8.5 GiB** vs the ~2.4 GiB `airbyte-values.yaml` budgets. Plus Magento replication at 5 Gi source + 5 Gi dest on a 15.25 GiB node → kernel `global_oom` (`constraint=CONSTRAINT_NONE`). BestEffort meant the kernel picked the *control plane* as victim (restarts: launcher 45, worker 33, server 8) — the worst choice, since a dead `airbyte-server` breaks the API, the UI and remediation's ability to restart anything. Fixed by bounding all 8 pods; host available went 5.3 → 8.6 GiB. **`airbyte-db` is live-patch-only** — the chart exposes no `postgresql.resources` key, so it reverts on reinstall.
2. **Auto-remediation could only kill, never restart** — see the public-API NPE note above.
3. **Alert thrash from a threshold below normal data age.** Global 25/30 min thresholds vs measured connection-level extract gaps: Magento p50 9 / p90 10 / p99 20 min (healthy — only 4 gaps >25 min in 7 days), Fishbowl p50 10 / p90 40 / p99 60 min. Fishbowl's tail is **business-hours seasonality, not a fault**: 02–09 UTC (overnight US) averages 27–53 min because there is genuinely nothing to extract, and the monitor cannot distinguish "no changes" from "frozen". 126 of 544 Fishbowl gaps breached 25 min → ~27 alert emails in 26h. Now per-connection: `magento_s3` 30/45, `fishbowl_s3` 60/90 — simulated over 7 days that is ~6 fires/week instead of ~130, and the 4.5h outage of 2026-07-15 would still trip ALERT comfortably.

**Net effect of #1–#2:** the Lambda spent days cancelling healthy Magento syncs (a connection whose own p99 was 20 min) and leaving nothing running. `observe-only` set back to `true` pending a soak.
...
| Airbyte Incident Runbook | `docs/AIRBYTE_INCIDENT_RUNBOOK.md` | Cancel + restart playbook for stuck Airbyte syncs (5-min target, SSM-only EC2) |

---

## Tooling Inventory

Agents, KB, skills, rules, docs, and MCP tooling available to this repo are documented in [`.claude/CLAUDE.md`](./.claude/CLAUDE.md) (auto-generated by `/sync-context`; do not duplicate here).

### Most Relevant Agents for This Project

- **dbt-expert** — dbt model development, testing, and debugging
- **snowflake-expert** — Snowflake queries, architecture, RBAC, Cortex
