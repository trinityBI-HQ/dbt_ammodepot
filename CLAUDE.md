...
- **Airbyte Observability (2026-05-01)**: Page 6 (`6_Airbyte_Health.py`) monitors S3 Iceberg ingestion freshness. Bootstrap in `setup/07_airbyte_observability.sql` (ACCOUNTADMIN, idempotent) creates: `AIRBYTE_FRESHNESS_THRESHOLDS` config table (warn=25 min, alert=30 min — relaxed from 15/30 on 2026-05-04 because normal extract age routinely lands 15–24 min and was firing WARN every cycle; was 30/60 originally, tightened on 2026-05-03; tunable via UPDATE), `V_AIRBYTE_FRESHNESS` + `V_AIRBYTE_FRESHNESS_PER_STREAM` views over all 55 LAKEHOUSE_LANDING tables, `SP_SEND_AIRBYTE_FRESHNESS_EMAIL(tier)` procedure, and two edge-triggered ALERT objects (`ALERT_AIRBYTE_FRESHNESS_WARN` / `ALERT_AIRBYTE_FRESHNESS_ALERT`) on `ETL_WH` at `cron(5,20,35,50)`. Emails via `OPS_EMAIL_NOTIFICATIONS` (account-level EMAIL integration, recipient `victor@trinitybi.com`). Runbook: `docs/AIRBYTE_INCIDENT_RUNBOOK.md`.
...
- **Alerting**: 2 edge-triggered Snowflake `ALERT` objects (`ALERT_AIRBYTE_FRESHNESS_WARN` at 25 min, `ALERT_AIRBYTE_FRESHNESS_ALERT` at 30 min — relaxed from 15/30 on 2026-05-04 to stop WARN firing every cycle; was 30/60 originally) running on `ETL_WH` at `cron(5,20,35,50 * * * * UTC)` — same cadence as dbt build to piggyback on warm warehouse (~$0/mo incremental cost vs ~$216/mo for a dedicated XSMALL with cold-start minimums)
...
- **Bootstrap**: `streamlit_cost_monitor/setup/07_airbyte_observability.sql` (ACCOUNTADMIN, idempotent) creates `AIRBYTE_FRESHNESS_THRESHOLDS` config table (warn=25 min, alert=30 min — relaxed from 15/30 on 2026-05-04 because normal extract age routinely lands 15–24 min; tunable via `UPDATE`), `V_AIRBYTE_FRESHNESS` + `V_AIRBYTE_FRESHNESS_PER_STREAM` views over all 55 LAKEHOUSE_LANDING tables, `SP_SEND_AIRBYTE_FRESHNESS_EMAIL(tier)` procedure, two edge-triggered alerts (`ALERT_AIRBYTE_FRESHNESS_WARN` / `ALERT_AIRBYTE_FRESHNESS_ALERT`) on `ETL_WH` at `cron(5,20,35,50)` to piggyback on warm warehouse
