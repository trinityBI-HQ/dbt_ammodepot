...
- **Audit log**: `AD_ANALYTICS.OPS.AIRBYTE_REMEDIATION_LOG` (bootstrap: `streamlit_cost_monitor/setup/08_airbyte_remediation_log.sql`) — one row per AUTO_FIX / ESCALATE / BREAKER_OPEN / OBSERVE_ONLY_WOULD_ACT event. WARN tier and OK invocations stay in CloudWatch only.
...
33 specialized agents in 9 categories (`.claude/agents/`):

| Category | Agents | Use When |
|---|---|---|
| **Data Engineering** (7) | dbt-expert, analytics-engineer, snowflake-expert, medallion-architect, spark-specialist, dagster-expert, lakeflow-architect | dbt models, Snowflake queries, pipeline architecture, Spark jobs, DLT, BI/metrics layer |
| **Code Quality** (7) | code-reviewer, code-documenter, python-developer, test-generator, audit-validator, code-cleaner, shell-script-specialist | Reviews, testing, documentation, audits, code cleaning, shell scripts |
| **Workflow** (6) | brainstorm-agent, define-agent, design-agent, build-agent, iterate-agent, ship-agent | SDD pipeline stages |
| **AI/ML** (4) | ai-data-engineer, ai-prompt-specialist, genai-architect, llm-specialist | AI system design, prompt engineering, GenAI architecture |
| **Communication** (3) | the-planner, adaptive-explainer, meeting-analyst | Planning, stakeholder explanations, meeting notes |
| **Dev** (2) | prompt-crafter, dev-loop-executor | PROMPT.md creation, Dev Loop execution |
| **Exploration** (2) | codebase-explorer, kb-architect | Codebase analysis, knowledge base management |
| **Automation** (1) | streamlit-expert | Streamlit apps, SiS deployment, dashboard optimization |
| **DevOps/SRE** (1) | ci-cd-specialist | Azure DevOps, Terraform, CI/CD pipelines |
...
## Knowledge Base (51 technologies in 6 categories)

| Category | Technologies | Key Technologies |
|---|---|---|
| data-engineering | 16 | dbt-core, dbt-cloud, dagster, snowflake, apache-iceberg, airbyte, duckdb, onehouse, great-expectations, soda, elementary, data-vault, data-contracts, openmetadata, finops, flake8 |
| devops-sre | 12 | terraform, terragrunt, kubernetes, docker-compose, grafana, prometheus, uv, python, github, testing, codebase-exploration, railway |
| cloud | 12 | S3, S3-tables, IAM, Glue, Athena, CloudWatch, KMS, Secrets Manager, Fargate, EMR, GCP, Lambda |
| ai-ml | 6 | pydantic, crewai, langfuse, langflow, gemini, openrouter |
| automation | 4 | streamlit, n8n, mermaid, clickup |
| document-processing | 1 | docling |
...
**Core:** `/memory`, `/readme-maker`, `/sync-context`, `/audit`, `/enrich-kb`, `/modernize`
...
**ClickUp Sync:** `/sync-task`, `/link-task`

---

## Rules

Path-scoped instruction files in `.claude/rules/` (11 files):
- **kb-development.md** — KB file conventions and size limits
- **kb-enrichment.md** — Route non-obvious session learnings back into the KB
...
## Delivery Standards (6 documents in `.claude/docs/`)
...
| `CLICKUP_TEMPLATE_SETUP.md` | ClickUp folder template + Tracks/Updates wiring (operational doc) |
