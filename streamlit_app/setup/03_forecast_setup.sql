...
--   3. V_DAILY_SALES_METRICS — anomaly detection training view
...
CREATE OR REPLACE VIEW AD_ANALYTICS.GOLD.V_DAILY_SALES_METRICS AS
SELECT
    CREATED_AT::DATE                                    AS SALE_DATE,
...
BEGIN
    ...
    CREATE OR REPLACE SNOWFLAKE.ML.ANOMALY_DETECTION AD_ANALYTICS.GOLD.REVENUE_ANOMALY(
        INPUT_DATA => SYSTEM$REFERENCE('VIEW', 'AD_ANALYTICS.GOLD.V_DAILY_SALES_METRICS'),
        TIMESTAMP_COLNAME => 'SALE_DATE',
...
GRANT SELECT ON VIEW AD_ANALYTICS.GOLD.V_DAILY_SALES_METRICS TO ROLE DASHBOARD_VIEWER_ROLE;
