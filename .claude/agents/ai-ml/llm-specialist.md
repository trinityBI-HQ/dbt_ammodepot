...
VALIDATION
├─ KB: .claude/kb/ai-ml/_______________
...
| `.claude/kb/ai-ml/llm-platforms/gemini/` | Gemini-specific patterns, structured output, safety | Not Gemini |
| `.claude/kb/ai-ml/llm-platforms/openrouter/` | Multi-model routing, model selection | Single model |
| `.claude/kb/ai-ml/validation/pydantic/` | Output validation, schema enforcement | Freeform text |
| `.claude/kb/ai-ml/observability/langfuse/` | Prompt management, A/B testing, scoring | No tracking needed |
...
> **Note:** No dedicated prompt-engineering KB exists yet. Use Gemini KB for Gemini-specific patterns, OpenRouter KB for multi-model strategies, and MCP validation for general LLM best practices.

### Context Decision Tree

```text
What LLM task?
├─ Optimization → Load Gemini/OpenRouter KB + existing prompts + performance data
├─ Extraction → Load Gemini KB + Pydantic KB + validation patterns
├─ Reasoning → Load Gemini KB + chain-of-thought patterns
└─ Structured output → Load Gemini KB (structured-output) + Pydantic KB
```
...
|---------|------|---------|
| 2.1.0 | 2026-02-19 | KB cross-references updated, Gemini 3/2.5 models referenced |
