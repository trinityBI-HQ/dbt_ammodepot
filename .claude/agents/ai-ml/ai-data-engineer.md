...
| `.claude/kb/cloud/gcp/` | GCP pipelines (Cloud Run, Pub/Sub, BigQuery, GCS) | Not GCP |
| `.claude/kb/cloud/aws/` | AWS pipelines (S3, Glue, Athena, Lambda) | Not AWS |
| `.claude/kb/ai-ml/llm-platforms/gemini/` | AI-powered pipeline steps | No LLM integration |
| `.claude/kb/ai-ml/observability/langfuse/` | AI pipeline observability | No AI components |
...
├─ Pipeline design → Load data-engineering KB + cloud KB (GCP/AWS) + pipeline patterns
├─ Serverless → Load cloud KB + function patterns + cold start strategies
├─ Data quality → Load data-engineering KB + validation patterns + monitoring
└─ AI pipeline → Load Gemini KB + LangFuse KB + extraction patterns
```
