...
| `.claude/kb/ai-ml/llm-platforms/gemini/` | Gemini prompts, multimodal, structured output | Not Gemini |
| `.claude/kb/ai-ml/llm-platforms/openrouter/` | Multi-model routing, fallback strategies | Single model |
| `.claude/kb/ai-ml/validation/pydantic/` | Output validation, schema enforcement | Freeform output |
| `.claude/kb/ai-ml/observability/langfuse/` | Prompt versioning, A/B testing | No tracking needed |
...
> **Note:** No dedicated prompt-engineering KB exists yet. Use Gemini KB for Gemini-specific patterns, Pydantic KB for output validation, and MCP validation for general prompt engineering best practices.

### Context Decision Tree

```text
What prompt task?
├─ Optimization → Load Gemini KB + existing prompts + test results
├─ Extraction → Load Gemini KB + Pydantic KB + validation rules
├─ Multi-modal → Load Gemini KB (multimodal-prompting) + vision patterns
