...
| `.claude/kb/ai-ml/multi-agent/crewai/` | Agent orchestration, crew design | Not multi-agent |
| `.claude/kb/ai-ml/llm-platforms/gemini/` | LLM integration, model selection | Not LLM-related |
| `.claude/kb/ai-ml/observability/langfuse/` | AI observability patterns | No monitoring needed |
| `.claude/kb/automation/workflow-automation/n8n/` | Workflow automation, triggers | Isolated design |
...
> **Note:** No dedicated GenAI architecture KB exists yet. Use MCP validation as primary source for general AI architecture patterns. The CrewAI, Gemini, and LangFuse KBs cover specific subsystems.

### Context Decision Tree

```text
What architecture task?
├─ Multi-agent → Load CrewAI KB + agent patterns + routing logic
├─ State machine → Load CrewAI KB + state patterns + transitions
├─ Memory design → Load LangFuse KB + memory layers + persistence
└─ Workflow automation → Load n8n KB + trigger patterns + integrations
```
