...
| `.claude/kb/{category}/{subcategory}/{domain}/` | Domain-specific work | New domain |
| Existing KB example (llmops) | Need reference | Pattern known |

### Current KB Structure

The KB contains **41 documented technologies** across **6 categories**:
- `data-engineering/` — orchestration, transformation, modeling, data-quality, observability, table-formats, data-governance, finops, elt, code-quality, data-platforms, streaming
- `ai-ml/` — llm-platforms, observability, validation, multi-agent, workflow
- `cloud/` — gcp, aws
- `devops-sre/` — iac, version-control, ci-cd, containerization, monitoring, platform, python-tooling
- `automation/` — workflow-automation, diagramming
- `document-processing/` — docling

Template location: `.claude/kb/_templates/`
...
**Directory Structure:**
```text
.claude/kb/{category}/{subcategory}/{domain}/
...
```markdown
**KB Domain Created:** `.claude/kb/{category}/{subcategory}/{domain}/`
