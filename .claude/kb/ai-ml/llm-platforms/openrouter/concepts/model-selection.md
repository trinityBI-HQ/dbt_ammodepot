...
OpenRouter provides access to 500+ models from 60+ providers. Models use `provider/model-name` format. Dynamic routing variants (`:nitro`, `:floor`) modify selection. The **Auto Router** (`openrouter/auto`, powered by NotDiamond) selects the best model per prompt. New provider routing fields: `zdr` (zero data retention), `sort` (price/throughput/latency), `quantizations` filter.
