...
Langfuse v3 automatically calculates costs for generations at ingestion time when usage data and pricing information are available. Cost data is stored in ClickHouse for fast aggregation and analytics. Built-in support covers OpenAI, Anthropic, and Google models. For custom models, you can configure pricing via the UI or API.
...
| `reasoning_tokens` | Reasoning tokens (o1/o3) | `300` |
...
| Cost over time | Time series | Trend analysis (ClickHouse) |

## v3 Analytics

In v3, cost queries run against ClickHouse OLAP, enabling sub-second aggregations across millions of traces. Use table filters and the API filters (Nov 2025) for granular cost breakdowns.
