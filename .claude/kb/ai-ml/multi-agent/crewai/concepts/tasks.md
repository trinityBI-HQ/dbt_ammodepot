...
> **Purpose**: Units of work with clear descriptions, expected outputs, and structured response formats
> **Confidence**: 0.95
> **MCP Validated**: 2026-02-19

## Overview

Tasks define specific work units for agents to complete in CrewAI v1.9.x. Each task includes a description of what needs to be done and an expected output format. Tasks can depend on other tasks through context, enabling data flow between agents. Since v1.9.0, tasks support structured outputs via `response_format` across all LLM providers and multimodal inputs (images, audio).
...
| `output_pydantic` | No | Return validated Pydantic object |
| `output_file` | No | Save output to file path |
| `images` | No | List of image paths (v1.9.0 multimodal) |

## Structured Outputs (v1.9.0)

Use Pydantic models for validated, typed outputs across any LLM provider:
...
triage_task = Task(
    description="Analyze logs at {log_path}...",
    expected_output="Structured error report",
    agent=triage_agent,
    output_pydantic=TriageResult  # Enforced structure
)
```
...
| Option | Use Case |
|--------|----------|
| `output_json=Model` | JSON dict validated with Pydantic |
| `output_pydantic=Model` | Return Pydantic object directly |
