...
**Deprecation (v0.99.0):** Internal tflint integration is deprecated. Use the
external `tflint` binary directly via hooks instead.
...
### External tflint (replaces deprecated internal tflint)

```hcl
terraform {
  after_hook "tflint" {
    commands = ["plan"]
    execute  = ["tflint", "--init", "&&", "tflint"]
  }
}
```
