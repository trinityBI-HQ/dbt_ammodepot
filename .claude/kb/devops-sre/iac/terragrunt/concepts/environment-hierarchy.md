...
**Alternative (v0.78.0+):** Stacks (`terragrunt.stack.hcl`) can replace copy-pasted
`terragrunt.hcl` files. See the Stacks section below.
...
Avoid flat structures like `dev-cloud-run/terragrunt.hcl`, `prod-cloud-run/terragrunt.hcl`.
Use hierarchical `environments/dev/cloud-run/terragrunt.hcl` or Stacks instead.

## Stacks Alternative (v0.78.0+)

Instead of copying `terragrunt.hcl` per environment, define a stack once:

```hcl
# infrastructure/terragrunt.stack.hcl
unit "vpc" {
  source = "./modules/vpc"
...
Then generate: `terragrunt stack generate`
