...
> **Purpose**: Define execution order between Terraform/OpenTofu units and pass outputs
> **Confidence**: 0.95
...
units (each unit = one Terraform/OpenTofu module instance). It builds a directed
acyclic graph (DAG) and ensures units are applied in the correct order. Dependencies
also enable passing outputs between units.
...
Target a subset of units instead of all. `--filter` implies `--all`.

```bash
terragrunt run-all apply --filter "path:vpc/*"  # Units matching path
terragrunt run-all plan --filter "git:main"     # Units changed vs main
terragrunt run-all apply --filter "tag:team=platform"  # By tag
terragrunt run-all plan --filter "dep:modules/vpc"     # Dependents of vpc
```
