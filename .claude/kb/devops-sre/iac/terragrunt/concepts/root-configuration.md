...
> **Purpose**: Central terragrunt.hcl that defines shared settings for all units
> **Confidence**: 0.95
...
remote state, provider generation, and common settings inherited by all child units.
This eliminates repetition across environments and units.

**Important (v0.81.0+):** All `include` blocks must have labels.
`include "root" { ... }` is required; bare `include { ... }` is deprecated and will
break in v2.0.
