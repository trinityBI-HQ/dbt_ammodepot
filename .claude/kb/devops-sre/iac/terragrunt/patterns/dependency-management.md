...
> **Purpose**: Orchestrate unit execution order and pass outputs between units
...
```bash
# Apply all units in correct order
terragrunt run-all apply

# Destroy in reverse order
terragrunt run-all destroy

# Apply single unit (dependencies auto-fetched)
cd cloud-run && terragrunt apply

# Target specific units with --filter (v0.98.0+)
terragrunt run-all apply --filter "path:cloud-run/*"
terragrunt run-all plan --filter "dep:vpc"       # All units depending on vpc
terragrunt run-all apply --filter "git:main"     # Only Git-changed units
```
