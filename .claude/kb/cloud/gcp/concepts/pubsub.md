...
Cloud Pub/Sub is a fully managed messaging service that decouples publishers from subscribers. Publishers send messages to topics; subscribers receive messages through subscriptions. It supports at-least-once and **exactly-once delivery** (GA April 2025) and handles millions of messages per second.
...
| **BigQuery** | Direct to warehouse (at-least-once only) | Audit trail |

## Delivery Guarantees

| Mode | Status | Notes |
|------|--------|-------|
| At-least-once | GA (default) | Standard, all subscription types |
| Exactly-once | GA (April 2025) | Pull subscriptions only, uses `ack_id` |

## Single Message Transforms (SMTs) - Preview

Inline transforms applied in subscriptions before delivery. No separate processing service needed.

```yaml
# Transform messages within subscription (conceptual)
# - Filter fields, rename keys, convert formats
# - Applied server-side before push/pull delivery
```

## Deprecation Notice

**GCS Object Change Notifications deprecated (Jan 2026).** Use Pub/Sub notifications for GCS instead: `gcloud storage buckets notifications create`.
