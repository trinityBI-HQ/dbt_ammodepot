...
Pub/Sub **exactly-once delivery** (GA April 2025) can be enabled on pull subscriptions for critical stages. For push subscriptions (used by Cloud Run), use idempotent handlers with deduplication via `message_id`. BigQuery subscriptions support at-least-once only.
