...
## Single Message Transforms (Preview)

SMTs allow inline message transformation within subscriptions before delivery. This can reduce the need for lightweight Cloud Run functions that only reshape data.

```text
[Publisher] → [Topic] → [Subscription + SMT (filter/rename/convert)] → [Consumer]
```

Use SMTs when consumers need a subset of fields or a different format than what publishers produce.
