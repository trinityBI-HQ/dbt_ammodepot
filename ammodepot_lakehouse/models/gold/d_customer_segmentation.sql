...
customer_sales as (
    select
        ...
        datediff(
          'day',
          max(fs.created_at),
          date_trunc('month', {{ convert_tz('UTC', var("ammodepot_timezone"), 'current_date') }}) - interval '1 day'
        )                                             as days_since_last_purchase
    from {{ ref('f_sales') }}           as fs
    inner join customer_base                 as cb
      on fs.customer_email = cb.customer_email
    where
      fs.created_at >= date_trunc('month', {{ convert_tz('UTC', var("ammodepot_timezone"), 'current_date') }}) - interval '1 year'
